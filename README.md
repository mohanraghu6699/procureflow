...
| **3. Purchase Order** | Created from an approved PR (one active PO per PR, amount capped at the approved amount, vendor must supply the category); PR-PO link and PO detail/status pages | 12 API tests |
...
| **6. Backend & database** | FastAPI (34 operations, Swagger at `/docs`), PostgreSQL 16, SQLAlchemy models with relationships, Alembic migrations, consistent error shapes | 139 tests on SQLite and PostgreSQL, run in CI |
...
| **8. Security** | JWT + bcrypt; Requester / Approver / Admin roles enforced in the API and in the UI routes; requesters see only their own records | auth and role tests, plus checks against the live site |
...
| Testing | pytest + FastAPI `TestClient` (139 API tests; SQLite by default, PostgreSQL in CI) |
