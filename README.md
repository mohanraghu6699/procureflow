...
| **6. Backend & database** | FastAPI (39 operations, Swagger at `/docs`), PostgreSQL 16, SQLAlchemy models with relationships, Alembic migrations, consistent error shapes | 186 tests on SQLite and PostgreSQL, run in CI |
...
| Testing | pytest + FastAPI `TestClient` (186 API tests; SQLite by default, PostgreSQL in CI) |
...
- **Starting a demo from a clean slate:** `bash deploy/gcp/update.sh --reset-data` deploys as usual and then deletes every purchase request, purchase order, delivery and status-history entry from the live database, so the demo starts empty and the numbering restarts at `PR-<year>-0001` / `PO-<year>-0001`. Accounts, departments, categories and vendors are kept. It asks you to type `RESET` before doing anything (`CONFIRM_RESET=RESET` skips the prompt in a script), runs as a one-off Cloud Run job that is removed afterwards, and finally checks the admin's dashboard is empty. Use it with `all` or `api`, and it combines with `--sync-passwords`. It can be repeated for each rehearsal. On a local database, run `python -m app.reset_data` from `backend/` (it shows what it will delete and asks for `RESET`; `--yes` skips the prompt). It is a command rather than a migration on purpose: a migration runs once per database and would wipe every database that upgrades past it.
