...
def update_purchase_request(
    ...
    if "category_id" in data and not db.query(Category).filter(Category.id == data["category_id"]).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid category_id")
    if "vendor_id" in data or "category_id" in data:
        effective_vendor_id = data["vendor_id"] if "vendor_id" in data else pr.vendor_id
        effective_category_id = data.get("category_id", pr.category_id)
